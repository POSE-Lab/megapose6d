...
def load_observation(
    ...
    camera_data = CameraData.from_bop(example_dir / "camera.json")

    rgb = np.array(Image.open(example_dir / "test" / "000002" / "rgb" / "000000.png"), dtype=np.uint8)
    assert rgb.shape[:2] == camera_data.resolution

    depth = None
    if load_depth:
        depth = np.array(Image.open(example_dir / "test" / "000002" / "depth" / "000000.png"), dtype=np.float32) / 1000
...
if __name__ == "__main__":
    ...
    example_dir = BOP_DS_DIR / args.example_name
